...
class TCPServer
{
    ...
    bool dealwithpacket(int conn_fd,unsigned char *recv_data,uint16_t wOpcode,int datasize);
     ...
     bool server_send(int conn_fd,unsigned char *send_buf,int datasize,uint16_t wOpcode); //发送数据函数
...
bool TCPServer::dealwithpacket(int conn_fd,unsigned char *recv_data,uint16_t wOpcode,int datasize)  //处理接收到的数据
{
    ...
    { 
        ...
        string filename =file_message->filename;
        ...
        if(stat(file_message->filename,&file_buffer)<0)
        {
            cout<<"获取文件信息失败"<<endl;
        }
        file_message->fseek=file_buffer.st_size;
        strcpy(file_message->filename,filename.c_str());
        
        if(server_send(conn_fd,(unsigned char*)file_message,sizeof(File_message),NET_TEST1)<0)
        {
...
bool TCPServer::server_send(int conn_fd,unsigned char *send_buf,int datasize,uint16_t wOpcode) //发送数据
{
